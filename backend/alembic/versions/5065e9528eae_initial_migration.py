...
Revision ID: 5065e9528eae
Revises: 
Create Date: 2026-03-13 00:06:47.131068
...
def upgrade() -> None:
    ...
    op.alter_column('locations', 'campus',
               existing_type=sa.VARCHAR(length=100),
               nullable=False,
               existing_server_default=sa.text("'UAO'::character varying"))
    ...
    op.alter_column('locations', 'label',
               ...
               comment='Ej: Cafetería Central, Sinapsis',
    ...
    op.drop_table_comment(
        'locations',
        existing_comment='Ubicación de vendedores con coordenadas PostGIS (SRID 4326).',
        schema=None
    )
    op.alter_column('negotiations', 'status',
               existing_type=postgresql.ENUM('pending', 'accepted', 'rejected', 'completed', name='negotiation_status'),
               type_=sa.Enum('pending', 'accepted', 'rejected', 'completed', name='negotiation_status_enum'),
               nullable=False,
               existing_server_default=sa.text("'pending'::negotiation_status"))
    op.alter_column('negotiations', 'created_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               nullable=False,
               existing_server_default=sa.text('now()'))
    op.alter_column('negotiations', 'updated_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               nullable=False,
               existing_server_default=sa.text('now()'))
    op.drop_index(op.f('idx_negotiations_buyer_id'), table_name='negotiations')
    op.drop_index(op.f('idx_negotiations_seller_id'), table_name='negotiations')
    op.drop_index(op.f('idx_negotiations_status'), table_name='negotiations')
    op.drop_table_comment(
        'negotiations',
        existing_comment='Registro de negociaciones entre compradores y vendedores.',
        schema=None
    )
    ...
    op.alter_column('otp_codes', 'attempts',
    ...
    op.alter_column('otp_codes', 'created_at',
               ...
               nullable=False,
               existing_server_default=sa.text('now()'))
    op.drop_index(op.f('idx_otp_codes_email'), table_name='otp_codes')
    op.drop_index(op.f('idx_otp_codes_expires'), table_name='otp_codes')
    op.create_index(op.f('ix_otp_codes_email'), 'otp_codes', ['email'], unique=False)
    op.drop_table_comment(
        'otp_codes',
        existing_comment='Códigos OTP de un solo uso para autenticación sin contraseña (HU 1.1).',
        schema=None
    )
    op.alter_column('products', 'price',
               existing_type=sa.NUMERIC(precision=12, scale=2),
               comment='Precio en COP',
               existing_comment='Precio en COP. Debe ser mayor a 0.',
    ...
    op.add_column('users', sa.Column('vendor_status', sa.Enum('pending', 'approved', 'rejected', name='vendor_status_type'), server_default='pending', nullable=False))
    op.add_column('users', sa.Column('sinapsis_code', sa.String(length=50), nullable=True))
    op.alter_column('users', 'email',
               existing_type=sa.VARCHAR(length=255),
               comment=None,
               existing_comment='Solo correos institucionales .edu.co (Ley 1581/2012).',
               existing_nullable=False)
    ...
    op.alter_column('users', 'is_verified',
               ...
               nullable=False,
               existing_server_default=sa.text('false'))
    op.alter_column('users', 'show_email',
               existing_type=sa.BOOLEAN(),
               nullable=False,
               comment=None,
               existing_comment='Privacidad: mostrar email en perfil público (HU 1.3).',
               existing_server_default=sa.text('false'))
    op.alter_column('users', 'show_phone',
               existing_type=sa.BOOLEAN(),
               nullable=False,
               comment=None,
               existing_comment='Privacidad: mostrar teléfono en perfil público (HU 1.3).',
               existing_server_default=sa.text('false'))
    op.alter_column('users', 'accepted_terms_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               comment=None,
               existing_comment='Registro de aceptación de términos según Ley 1581.',
               existing_nullable=True)
    ...
    op.alter_column('users', 'updated_at',
               ...
               nullable=False,
               existing_server_default=sa.text('now()'))
    op.drop_index(op.f('idx_users_institutional_id'), table_name='users')
    op.create_unique_constraint(None, 'users', ['sinapsis_code'])
...
def downgrade() -> None:
    ...
    op.create_index(op.f('idx_users_institutional_id'), 'users', ['institutional_id'], unique=False)
    op.alter_column('users', 'updated_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               nullable=True,
               existing_server_default=sa.text('now()'))
    ...
    op.alter_column('users', 'accepted_terms_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               comment='Registro de aceptación de términos según Ley 1581.',
               existing_nullable=True)
    op.alter_column('users', 'show_phone',
               existing_type=sa.BOOLEAN(),
               nullable=True,
               comment='Privacidad: mostrar teléfono en perfil público (HU 1.3).',
               existing_server_default=sa.text('false'))
    op.alter_column('users', 'show_email',
               existing_type=sa.BOOLEAN(),
               nullable=True,
               comment='Privacidad: mostrar email en perfil público (HU 1.3).',
               existing_server_default=sa.text('false'))
    op.alter_column('users', 'is_verified',
               existing_type=sa.BOOLEAN(),
               nullable=True,
    ...
    op.alter_column('users', 'email',
               existing_type=sa.VARCHAR(length=255),
               comment='Solo correos institucionales .edu.co (Ley 1581/2012).',
               existing_nullable=False)
    op.drop_column('users', 'sinapsis_code')
    ...
    op.alter_column('products', 'price',
               existing_type=sa.NUMERIC(precision=12, scale=2),
               comment='Precio en COP. Debe ser mayor a 0.',
               existing_comment='Precio en COP',
               existing_nullable=False)
    op.create_table_comment(
        'otp_codes',
        'Códigos OTP de un solo uso para autenticación sin contraseña (HU 1.1).',
        existing_comment=None,
        schema=None
    )
    op.drop_index(op.f('ix_otp_codes_email'), table_name='otp_codes')
    op.create_index(op.f('idx_otp_codes_expires'), 'otp_codes', ['expires_at'], unique=False)
    op.create_index(op.f('idx_otp_codes_email'), 'otp_codes', ['email'], unique=False)
    op.alter_column('otp_codes', 'created_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               nullable=True,
               existing_server_default=sa.text('now()'))
    ...
    op.create_table_comment(
        'negotiations',
        'Registro de negociaciones entre compradores y vendedores.',
        existing_comment=None,
        schema=None
    )
    op.create_index(op.f('idx_negotiations_status'), 'negotiations', ['status'], unique=False)
    op.create_index(op.f('idx_negotiations_seller_id'), 'negotiations', ['seller_id'], unique=False)
    op.create_index(op.f('idx_negotiations_buyer_id'), 'negotiations', ['buyer_id'], unique=False)
    op.alter_column('negotiations', 'updated_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               nullable=True,
               existing_server_default=sa.text('now()'))
    op.alter_column('negotiations', 'created_at',
               ...
               existing_server_default=sa.text('now()'))
    op.alter_column('negotiations', 'status',
               existing_type=sa.Enum('pending', 'accepted', 'rejected', 'completed', name='negotiation_status_enum'),
               type_=postgresql.ENUM('pending', 'accepted', 'rejected', 'completed', name='negotiation_status'),
               nullable=True,
               existing_server_default=sa.text("'pending'::negotiation_status"))
    op.create_table_comment(
        'locations',
        'Ubicación de vendedores con coordenadas PostGIS (SRID 4326).',
        existing_comment=None,
        schema=None
    )
    op.alter_column('locations', 'updated_at',
    ...
    op.alter_column('locations', 'label',
               existing_type=sa.VARCHAR(length=200),
               comment=None,
               existing_comment='Ej: Cafetería Central, Sinapsis',
               existing_nullable=True)
    op.alter_column('locations', 'coordinates',
               existing_type=geoalchemy2.types.Geometry(geometry_type='POINT', srid=4326, dimension=2, from_text='ST_GeomFromEWKT', name='geometry', _spatial_index_reflected=True),
               nullable=True)
