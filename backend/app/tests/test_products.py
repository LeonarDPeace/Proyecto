...
async def test_list_products():
    ...
    fake_db_products = [
        MockProduct(**_make_product_dict()),
        MockProduct(**_make_product_dict()),
    ]

    with patch(
        "app.routers.products.product_service.list_products", new_callable=AsyncMock
    ) as mock_list:
...
async def test_get_product_success():
    ...
    with patch(
        "app.routers.products.product_service.get_product_by_id", new_callable=AsyncMock
    ) as mock_get:
...
async def test_create_product_vendedor():
    ...
    fake_prod = MockProduct(
        **_make_product_dict(seller_id=seller_id, name="Empanada", price=2500.0)
    )
    ...
    with patch(
        "app.routers.products.product_service.create_product", new_callable=AsyncMock
    ) as mock_create:
...
async def test_create_product_comprador_forbidden():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.post(
            "/api/v1/products/", json={"name": "Empanada", "price": 2500}
        )
...
async def test_create_product_invalid_price():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.post(
            "/api/v1/products/", json={"name": "Mal Precio", "price": -500}
        )
...
async def test_update_product_success():
    ...
    seller_id = uuid.uuid4()
    fake_prod = MockProduct(
        **_make_product_dict(id=prod_id, seller_id=seller_id, name="Nombre Actualizado")
    )
    ...
    with patch(
        "app.routers.products.product_service.update_product", new_callable=AsyncMock
    ) as mock_upd:
...
async def test_delete_product():
    ...
    with patch(
        "app.routers.products.product_service.soft_delete_product",
...
async def test_toggle_status_product():
    ...
    seller_id = uuid.uuid4()
    fake_prod = MockProduct(
        **_make_product_dict(id=prod_id, seller_id=seller_id, is_active=False)
    )
    ...
    with patch(
        "app.routers.products.product_service.toggle_product_status",
        new_callable=AsyncMock,
    ) as mock_tog:
